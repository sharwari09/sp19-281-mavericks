...
func incrementUserBookingEvent(request MyRequest) (MyResponse, error) {
	...
	for i := range dashboard.PostedEvents {
		if dashboard.PostedEvents[i].OrgID == eventID {
			bookings := dashboard.PostedEvents[i].NumberOfBookings
			dashboard.PostedEvents[i].NumberOfBookings = bookings + 1
...
func main() {
	lambda.Start(incrementUserBookingEvent)
}
